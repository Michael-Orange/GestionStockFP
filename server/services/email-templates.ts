...
export interface RappelRetardData {
  overdueLoans: Array<{
    productName: string;
    borrower: string;
    quantite: number;
...
/**
 * Template: Email rappel retards J+30
 */
export function createRappelRetardEmail(data: RappelRetardData): string {
  const { overdueLoans, date } = data;
  
  const itemsHtml = overdueLoans.map(loan => `
    <div class="item">
      <div class="item-name">
        ${loan.productName}
        <span class="badge badge-orange">${loan.daysSince} jours</span>
      </div>
      <div class="item-details">
        Emprunteur: ${loan.borrower} · ${loan.quantite} ${loan.unite}
      </div>
    </div>
  `).join('');
  ...
  return `
    ...
    <body>
      <div class="header">
        <h1>⏰ Rappel: Emprunts en Retard</h1>
        <p style="margin: 5px 0 0 0;">${date}</p>
      </div>
      <div class="content">
        <p style="margin-top: 0;">Les emprunts suivants sont en retard de plus de 30 jours:</p>
        <div class="section">
          <div class="section-title">⚠️ ${overdueLoans.length} emprunt(s) non retourné(s)</div>
          ${itemsHtml}
        </div>
        <p style="color: #dc2626; font-weight: 600;">Action recommandée: Contacter les emprunteurs</p>
      </div>
      <div class="footer">
        <strong>FiltrePlante</strong> - Gestion de Stock<br>
        Cet email est généré automatiquement, ne pas répondre
